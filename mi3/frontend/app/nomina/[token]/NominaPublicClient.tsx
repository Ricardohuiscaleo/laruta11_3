...
export default function NominaPublicClient({ params, initialData, initialMes, initialCreatedAt, initialAprobadoPor, initialAprobadoAt, initialConfirmados }: Props) {
  ...
  const [data, setData] = useState<SnapshotData | null>(initialData ?? null);
  const [createdAt, setCreatedAt] = useState(initialCreatedAt ?? '');
  const [aprobadoPor, setAprobadoPor] = useState<string | null>(initialAprobadoPor ?? null);
  const [aprobadoAt, setAprobadoAt] = useState<string | null>(initialAprobadoAt ?? null);
