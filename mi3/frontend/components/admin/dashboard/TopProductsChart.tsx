...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-lg bg-white border shadow-lg p-3 text-xs space-y-1 max-w-[200px]">
      ...
      <p className="text-gray-600">Costo: <span className="font-medium text-red-600">{fmtCLP(d.total_cost)}</span></p>
      <p className="text-gray-600">Utilidad: <span className="font-medium text-green-600">{fmtCLP(d.total_profit)}</span></p>
      <p className="text-gray-600">Margen: <span className="font-bold">{d.margin_pct}%</span></p>
      <p className="text-gray-600">Acumulado: <span className="font-bold">{d.cumulative_pct.toFixed(0)}%</span></p>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border bg-white shadow-sm p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-gray-900">Top Productos <span className="text-[10px] text-gray-400 font-normal">(Pareto)</span></h3>
      ...
      {loading ? (
        ...
        <ResponsiveContainer width="100%" height={Math.max(200, data.length * 34)}>
          <ComposedChart data={data} layout="vertical" margin={{ top: 5, right: 35, left: 0, bottom: 5 }}>
            ...
            <Bar dataKey="total_cost" name="Costo" stackId="a" fill="#FCA5A5" barSize={20}>
              {data.map((_, i) => <Cell key={i} fill="#FCA5A5" />)}
            </Bar>
            <Bar dataKey="total_profit" name="Utilidad" stackId="a" fill="#86EFAC" barSize={20}>
              {data.map((_, i) => <Cell key={i} fill="#86EFAC" />)}
            </Bar>
            <Line type="monotone" dataKey="cumulative_pct" name="Acumulado %" stroke="#F59E0B" strokeWidth={2} dot={{ r: 3, fill: '#F59E0B' }} />
          </ComposedChart>
        </ResponsiveContainer>
      )}
      {data.length > 0 && (
        <p className="text-[10px] text-gray-400 mt-2 text-center">
          Línea amarilla = % acumulado de ingresos (Pareto 80/20)
        </p>
      )}
    </div>
  );
