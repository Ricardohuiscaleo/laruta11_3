...
class VentasService
{
    ...
    public function getCmvIngredientProducts(int $ingredientId, string $period, ?string $month = null): array
    {
        ...
        // Also includes combos via combo_items → product_recipes
        $productMap = [];
        $seen = [];

        // Build combo mapping: product_id (category 8) → combo_id → items that use this ingredient
        $comboProductIds = DB::table('products')->where('category_id', 8)->pluck('id')->toArray();
        $comboIdByProductId = [];
        if (!empty($comboProductIds)) {
            // Match products (category 8) to combos by name similarity
            $combos = DB::table('combos')->where('active', 1)->get();
            $comboProductNames = DB::table('products')->whereIn('id', $comboProductIds)->pluck('name', 'id');
            foreach ($comboProductNames as $pid => $pname) {
                $pnorm = mb_strtolower(trim($pname));
                foreach ($combos as $combo) {
                    $cnorm = mb_strtolower(trim($combo->name));
                    if ($pnorm === $cnorm || str_contains($pnorm, $cnorm) || str_contains($cnorm, $pnorm)) {
                        $comboIdByProductId[$pid] = $combo->id;
                        break;
                    }
                }
            }
        ...
        // For each combo, calculate how many units of this ingredient it uses
        $comboIngredientQty = [];
        if (!empty($comboIdByProductId)) {
            $comboItems = DB::table('combo_items')
                ->whereIn('combo_id', array_values($comboIdByProductId))
                ->select('combo_id', 'product_id', 'quantity')
                ->get();
            foreach ($comboItems as $ci) {
                $itemRecipeQty = $recipes[$ci->product_id] ?? null;
                if ($itemRecipeQty === null) continue;
                $comboIngredientQty[$ci->combo_id] = ($comboIngredientQty[$ci->combo_id] ?? 0) + ((int) $ci->quantity * $itemRecipeQty);
            }
        }
        ...
        foreach ($transactions as $tx) {
            ...
            // Combo match: product is in category 8 and maps to a combo with items using this ingredient
            $comboId = $comboIdByProductId[$tx->product_id] ?? null;
            if ($comboId && isset($comboIngredientQty[$comboId])) {
                $key = 'combo-' . $tx->product_id;
                if (!isset($productMap[$key])) {
                    $productMap[$key] = [
                        'product_name' => $tx->product_name,
                        'times_sold' => 0,
                        'recipe_qty' => round($comboIngredientQty[$comboId], 3),
                    ];
                }
                $productMap[$key]['times_sold'] += $tx->item_qty;
            }
