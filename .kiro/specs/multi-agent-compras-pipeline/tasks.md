...
- [ ] 2. GeminiService — nuevos métodos de agentes
  ...
  - [~] 2.6 Write property test: Propiedad 1 — imagen procesada exactamente una vez
  ...
  - [~] 2.7 Write property test: Propiedad 2 — estructura de salida del Agente Visión
...
- [ ] 3. Checkpoint — Verificar métodos de GeminiService
  - Ejecutar tests unitarios de GeminiService, asegurar que compilan y pasan. Preguntar al usuario si hay dudas.

- [ ] 4. FeedbackService — motor de auto-aprendizaje
  - [~] 4.1 Crear `FeedbackService.php` con método `capturarFeedback(int $extractionLogId, ?int $compraId, array $datosGuardados): void`
    ...
    - _Requisitos: 6.1, 6.2_
  - [~] 4.2 Implementar `getFewShotExamples(?string $proveedor, ?string $tipoImagen, int $limit = 5): array`
  ...
  - [~] 4.3 Implementar `formatearEjemplos(array $corrections): string`
    ...
    - _Requisitos: 6.4_
  - [~] 4.4 Implementar `computeDiff(array $original, array $final): array`
    ...
    - _Requisitos: 6.1, 6.2_
  - [~] 4.5 Write property test: Propiedad 5 — inyección correcta de few-shot examples
  ...
  - [~] 4.6 Write property test: Propiedad 15 — diff de feedback captura todas las diferencias
...
- [ ] 5. PipelineExtraccionService — orquestador multi-agente
  - [~] 5.1 Implementar `ejecutarMultiAgente(string $imageUrl, ?callable $onEvent = null): array`
  ...
  - [~] 5.2 Implementar logging y cálculo de costos en ejecutarMultiAgente
  ...
  - [~] 5.3 Implementar detección automática de pipeline (env flag)
  ...
  - [~] 5.4 Write property test: Propiedad 6 — validación aritmética detecta inconsistencias
  ...
  - [~] 5.5 Write property test: Propiedad 7 — validación fiscal detecta IVA incorrecto
  ...
  - [~] 5.6 Write property test: Propiedad 10 — reconciliación sin inconsistencias es pass-through
  ...
  - [~] 5.7 Write property test: Propiedad 12 — eventos SSE emitidos en orden correcto
...
- [ ] 6. Checkpoint — Verificar pipeline backend completo
...
- [ ] 7. Frontend — ExtractionPipeline.tsx con 4 fases y reconciliación
  - [~] 7.1 Actualizar ExtractionPipeline.tsx para soportar 4 fases multi-agente
  ...
  - [~] 7.2 Implementar PhaseDetails para fases de validación y reconciliación
  ...
  - [~] 7.3 Implementar UI de preguntas de reconciliación
  ...
  - [~] 7.4 Actualizar page.tsx de registro para manejar flujo de reconciliación
...
- [ ] 8. Wiring — Captura de feedback en CompraController
  - [~] 8.1 Inyectar FeedbackService en CompraController
    - Agregar dependencia en constructor
    - _Requisitos: 6.1_
  - [~] 8.2 Capturar feedback al guardar compra en método `store()`
...
- [ ] 9. ExtraccionController — usar ejecutarMultiAgente
  - [~] 9.1 Actualizar `extractPipeline()` para usar ejecutarMultiAgente cuando disponible
...
- [ ] 10. Checkpoint final — Integración completa
