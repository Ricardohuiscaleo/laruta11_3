...
const CheckoutApp = ({ onClose }) => {
  ...
  // Delivery config from DB with fallback defaults
  ...
  const deliveryDiscountAmount = deliveryDiscountActive ? Math.round(deliveryFee * deliveryConfig.rl6_discount_factor) : 0;
  ...
  const processR11Payment = async () => {
    ...
    try {
      ...
      if (creditResult.success) {
        ...
        localStorage.removeItem('ruta11_cart');
        localStorage.removeItem('ruta11_cart_total');
        window.location.href = '/r11c-pending?order=' + orderResult.order_id;
      } else {
        setIsProcessingOrder(false);
        alert('❌ Error al usar crédito: ' + creditResult.error);
      }
                  ...
                  return (
                    <div key={index} className="border-b border-gray-100 pb-3 last:border-b-0">
                      <div className="flex gap-2 items-start">
                        <img
                          src={item.image_url || `https://pub-d6bf1ac3bcb0465cabadb9eeab426a65.r2.dev/WhatsApp%20Image%202026-05-20%20at%2019.50.28.jpeg`}
                          alt={item.name}
                          className="w-12 h-12 object-cover rounded-md border border-gray-200 flex-shrink-0"
                          onError={(e) => { e.target.src = `https://pub-d6bf1ac3bcb0465cabadb9eeab426a65.r2.dev/WhatsApp%20Image%202026-05-20%20at%2019.50.28.jpeg`; }}
                        />
