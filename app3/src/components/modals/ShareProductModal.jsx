...
import { X, Share2, Instagram, QrCode, Link } from 'lucide-react';
...
const ShareProductModal = ({ isOpen, onClose, product }) => {
  ...
  useEffect(() => {
    if (isOpen && product) {
      ...
      const productUrl = `${window.location.origin}/p.php?id=${product.id}`;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm z-50 flex justify-center items-end animate-fade-in" onClick={onClose}>
      <div className="bg-white w-full mx-1 rounded-t-2xl flex flex-col animate-slide-up max-h-[80vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-grow overflow-y-auto p-4">
          ...
          {/* Opciones de Compartir - 4 botones en 1 fila */}
          <div className="grid grid-cols-4 gap-2 mb-6">
            ...
            >
              ...
              <p className="font-semibold text-gray-800 text-[10px] text-center">WhatsApp</p>
            </button>

            <button
              onClick={handleInstagram}
              className="flex flex-col items-center gap-1 p-3 bg-gradient-to-r from-purple-50 to-pink-50 hover:from-purple-100 hover:to-pink-100 rounded-lg transition-colors"
            >
              <div className="w-10 h-10 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full flex items-center justify-center">
                <Instagram size={20} className="text-white" />
              </div>
              <p className="font-semibold text-gray-800 text-[10px] text-center">Instagram</p>
            </button>
            ...
            >
              {qrCode ? (
                <img src={qrCode} alt="QR Code" className="w-10 h-10 rounded" />
              ) : (
                <div className="w-10 h-10 bg-gray-500 rounded-full flex items-center justify-center">
                  <QrCode size={20} className="text-white" />
                </div>
              )}
              <p className="font-semibold text-gray-800 text-[10px] text-center">QR Code</p>
            </button>
            ...
            >
              ...
              <p className="font-semibold text-gray-800 text-[10px] text-center">Copiar Link</p>
            </button>
          </div>
