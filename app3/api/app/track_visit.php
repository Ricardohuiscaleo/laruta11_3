...
try {
    ...
    // Si falla, intentar crear las columnas faltantes (usando método compatible)
    try {
        $cols = [
        ...
        foreach ($cols as $col => $def) {
            $check = $pdo->query("SHOW COLUMNS FROM site_visits LIKE '$col'");
            if ($check->rowCount() == 0) {
                $pdo->exec("ALTER TABLE site_visits ADD COLUMN $col $def");
            }
        }

        echo json_encode(['success' => true, 'message' => 'Table updated and check completed']);
    }
    catch (Exception $e2) {
