...
try {
    ...
    if ($validate_httpCode !== 200) {
        error_log("TUU Validate Error - HTTP $validate_httpCode: $validate_response");
        throw new Exception("Error validando token TUU - HTTP $validate_httpCode");
    }
    
    $validate_result = json_decode($validate_response, true);
    if (!isset($validate_result['secret_key'])) {
        error_log("TUU Validate Response: $validate_response");
        throw new Exception('Error validando token TUU - secret_key no recibido');
    }
