...
if (isset($_GET['x_reference']) && isset($_GET['x_result']) && $_GET['x_result'] === 'completed') {
    ...
    try {
        ...
        if ($order) {
            ...
            $whatsapp_message .= "*📋 Datos del pedido:*\n";
                    ...
                    if ($item['combo_data']) {
                        ...
                        if (isset($combo_data['fixed_items']) || isset($combo_data['selections'])) {
                            ...
                            if (!empty($include_items)) {
                                foreach ($include_items as $inc) {
                                    $whatsapp_message .= "   - {$inc}\n";
                                }
                            }
                        }
                        ...
                        if (isset($combo_data['customizations'])) {
                            foreach ($combo_data['customizations'] as $custom) {
                                $whatsapp_message .= "   - {$custom['quantity']}x {$custom['name']}\n";
                            }
                        }
                    }
            ...
            if ($delivery_info) {
                $whatsapp_message .= "*🚚 Entrega:*\n";
                $whatsapp_message .= "- *Tipo:* " . ($delivery_info['delivery_type'] === 'delivery' ? '🚴 Delivery' : '🏪 Retiro en local') . "\n";
                ...
                if ($delivery_info['delivery_type'] === 'pickup' && $delivery_info['pickup_time']) {
                    $whatsapp_message .= "- *Hora retiro:* {$delivery_info['pickup_time']}\n";
                }
            ...
            $whatsapp_message .= "\n> *💰 TOTAL: $" . number_format($order['product_price'], 0, ',', '.') . "*\n\n";
            $whatsapp_message .= "_Pago confirmado con Webpay - Pedido desde la app web._";
