        ...
        <div className="flex items-center gap-3 w-full max-w-7xl mx-auto">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 text-xs text-gray-600">
              ...
              {checklist && (
                <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${checklist.status === 'completed' ? 'bg-green-100 text-green-700' :
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'apertura' ? 'border-orange-500 text-orange-600' : 'border-transparent text-gray-500'
                }`}
            >
              ☀️ 18:00
            </button>
            <button
              ...
              className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'cierre' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500'
                }`}
            >
              🌙 00:45
            </button>
            <button
              ...
              className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'historial' ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500'
                }`}
            >
              📅 Historial
            </button>
              ...
              <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      >
                        {item.is_completed ? (
                          ...
                          <Circle size={32} className="text-gray-200" />
                        )}
                      </button>
                      <div className="flex-1">
                        ...
                        {checklist.status !== 'completed' && (
                          <textarea
                            ...
                            className="w-full mt-3 p-3 text-sm border-gray-100 border bg-gray-50 rounded-xl resize-none focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none transition-all"
                            rows="2"
                        ...
                        {item.notes && checklist.status === 'completed' && (
                          <p className="text-sm text-gray-600 mt-2 italic bg-gray-50 p-2 rounded-lg">📝 {item.notes}</p>
                        )}
                        {!!item.requires_photo && (
                          <div className="mt-3">
                            {item.photo_url ? (
                              <div className="relative rounded-xl overflow-hidden shadow-md">
                                <img src={item.photo_url} alt="Evidencia" className="w-full h-48 object-cover" />
                                {checklist.status !== 'completed' && (
                                  <button
                                    onClick={() => {
                                    ...
                                    className="absolute top-2 right-2 bg-red-500/90 backdrop-blur-sm text-white p-2 rounded-full hover:bg-red-600 shadow-lg transition-all"
                                  >
                                    <X size={20} />
                                  </button>
                                )}
                              ...
                              <label className="flex items-center justify-center gap-2 bg-orange-50 text-orange-600 px-4 py-3 rounded-xl cursor-pointer hover:bg-orange-100 transition-all border border-orange-100 shadow-sm group">
                                <Camera size={20} className="group-hover:scale-110 transition-transform" />
                                <span className="font-bold">Subir Foto Evidencia</span>
                                <input
                                  type="file"
                                  accept="image/*"
                                  capture="environment"
                ...
                {checklist.status !== 'completed' && (
                  <button
                    ...
                    className="w-full bg-orange-500 text-white py-4 rounded-xl font-black uppercase tracking-widest mt-8 hover:bg-orange-600 shadow-xl active:scale-[0.98] transition-all"
                  >
                    Completar Checklist
                  </button>
                )}
              ...
              {checklists.apertura && (
                <div className="border rounded-lg overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <div className={`px-2 py-1 rounded-full text-xs font-semibold ${checklists.apertura.status === 'completed' ? 'bg-green-100 text-green-700' :
              ...
              {checklists.cierre && (
                <div className="border rounded-lg overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <div className={`px-2 py-1 rounded-full text-xs font-semibold ${checklists.cierre.status === 'completed' ? 'bg-green-100 text-green-700' :
