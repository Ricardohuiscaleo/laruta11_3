...
export default function PersonalApp() {
  ...
  function getLiquidacion(p, modoContexto = 'all') {
    ...
    const montoMultas = includeAjustes ? ajustesPer.filter(a => (a.tipo === 'multa' || (a.concepto || '').toLowerCase().includes('falta') || (a.concepto || '').toLowerCase().includes('multa')) && a.tipo !== 'reemplazo_caja').reduce((s, a) => s + Math.abs(parseFloat(a.monto)), 0) : 0;
    const montoCorrecciones = includeAjustes ? ajustesPer.filter(a => (a.tipo === 'correccion' || (a.concepto || '').toLowerCase().includes('febrero') || (a.concepto || '').toLowerCase().includes('anterior')) && a.tipo !== 'reemplazo_caja').reduce((s, a) => s + parseFloat(a.monto), 0) : 0;
    const montoReemplazosCaja = includeAjustes ? ajustesPer.filter(a => a.tipo === 'reemplazo_caja').reduce((s, a) => s + Math.abs(parseFloat(a.monto)), 0) : 0;
