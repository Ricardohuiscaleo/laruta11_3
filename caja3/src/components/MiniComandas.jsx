              ...
              {order.delivery_type === 'delivery' && order.delivery_fee > 0 && (
                <div className="bg-gray-50 rounded p-1.5 space-y-0.5">
                  ...
                  <div className="border-t border-gray-200 mt-1 pt-1 flex justify-between text-xs font-bold text-gray-800">
                    ...
                    <span>${parseInt(order.delivery_fee).toLocaleString('es-CL')}</span>
                  </div>
                </div>
              )}
              {order.delivery_type === 'delivery' && order.payment_method === 'card' && parseInt(order.card_surcharge || 0) > 0 && (
                <div className="flex justify-between text-xs text-red-500">
                  <span>💳 Recargo tarjeta</span>
                  <span className="font-semibold">+${parseInt(order.card_surcharge).toLocaleString('es-CL')}</span>
                </div>
              )}
              {order.discount_10 > 0 && (
