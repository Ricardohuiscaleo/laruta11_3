...
try {
    ...
    $stmt = $conn->prepare("
        ...
        FROM tuu_orders 
        WHERE order_number = ? AND LOWER(payment_method) = 'r11_credit'
    ");
    ...
    if (!$order) {
        throw new Exception('Pedido no encontrado o no pagado con crédito R11');
    }
    ...
    // 2. Reintegrar crédito (restar de credito_r11_usado, sin bajar de 0)
    $stmt = $conn->prepare("
        UPDATE usuarios 
