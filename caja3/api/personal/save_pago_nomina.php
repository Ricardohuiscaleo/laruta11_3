...
$centro_costo = $body['centro_costo'] ?? 'ruta11';
if (!$nombre || !$monto) {
    echo json_encode(['success' => false, 'error' => 'Faltan datos']);
    exit;
}
$stmt = mysqli_prepare($conn,
    "INSERT INTO pagos_nomina (mes, personal_id, nombre, monto, es_externo, notas, centro_costo)
     VALUES (?, ?, ?, ?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE monto=VALUES(monto), notas=VALUES(notas)"
);
mysqli_stmt_bind_param($stmt, 'sisssss', $mes, $personal_id, $nombre, $monto, $es_externo, $notas, $centro_costo);
