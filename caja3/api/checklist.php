...
if (!$action) {
    send_json(['success' => false, 'error' => 'Action required']);
}
...
function createDaily($conn)
{
    ...
    foreach (['apertura', 'cierre'] as $type) {
        ...
        if ($type === 'cierre') {
            $tomorrow = clone $current;
            $tomorrow->modify('+1 day');
            $checklist_date = $tomorrow->format('Y-m-d');
        }

        // Verificar si ya existe
        $stmt = $conn->prepare("SELECT id FROM checklists WHERE type = ? AND scheduled_date = ?");
        ...
        $stmt = $conn->prepare("
            INSERT INTO checklists (type, scheduled_time, scheduled_date, status, total_items, completed_items, completion_percentage)
            VALUES (?, ?, ?, 'pending', 0, 0, 0)
        ");
        $stmt->execute([$type, $scheduled_time, $checklist_date]);
        $checklist_id = $conn->lastInsertId();

        // Obtener template items
        $template_items = [
            'apertura' => [
                'Verificar limpieza de plancha',
            ...
            'cierre' => [
                ...
                'Guardar ingredientes en refrigerador',
                'Apagar todos los equipos',
                'Limpiar área de trabajo',
                'Verificar cierre de puertas y ventanas'
            ]
        ];

        // Insertar items
        $order = 1;
        foreach ($template_items[$type] as $description) {
            $stmt = $conn->prepare("
                INSERT INTO checklist_items (checklist_id, item_order, description, requires_photo, is_completed)
                VALUES (?, ?, ?, 0, 0)
            ");
            $stmt->execute([$checklist_id, $order, $description]);
            $order++;
        }

        // Actualizar total_items
        $stmt = $conn->prepare("UPDATE checklists SET total_items = ? WHERE id = ?");
        $stmt->execute([count($template_items[$type]), $checklist_id]);

        $created[] = ['type' => $type, 'date' => $checklist_date, 'id' => $checklist_id];
    }
