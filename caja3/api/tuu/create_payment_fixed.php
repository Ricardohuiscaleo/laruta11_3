...
try {
    ...
    if ($httpCode !== 200) {
        error_log("TUU Token Error - HTTP $httpCode: $token_response");
        throw new Exception("Error obteniendo token TUU - HTTP $httpCode");
    }
    
    $token_data = json_decode($token_response, true);
    if (!isset($token_data['token'])) {
        error_log("TUU Token Response: $token_response");
        throw new Exception('Token no recibido de TUU');
    }
    
    // PASO 2: Validar Token CON RETRY
    ...
    for ($retry = 0; $retry < $max_retries; $retry++) {
        ...
        error_log("TUU Validate Retry $retry - HTTP $validate_httpCode: $validate_response");
    }
    
    if (!isset($validate_result['secret_key'])) {
        throw new Exception('Error validando token TUU después de ' . $max_retries . ' intentos');
    }
